...
DataConnectionMock.prototype._configureDataChannel = function() {
    this.open = true;
    this.emit('open');

    if (this.peer in mocks) {
        this._connection = mocks[this.peer]._negotiate(this.provider.id);
    } else {
...
DataConnectionMock.prototype.close = function() {
    ...
    var _bufferedSend = function(msg) {
