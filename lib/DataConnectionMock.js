...
DataConnectionMock.prototype.constructor = DataConnectionMock;
...
DataConnectionMock.prototype._setRemote = function _setRemote(connection) {
...
DataConnectionMock.prototype._configureDataChannel = function _configureDataChannel(mocks) {
...
DataConnectionMock.prototype.send = function send(message, chunked) {
    if (!this.open) {
...
DataConnectionMock.prototype.receive = function receive(message) {
    this.emit('data', message);
};
DataConnectionMock.prototype.close = function close() {
    if (!this.open) {
        return;;
    }
    this.open = false;
